...
def deposit(amount):
    ...
    if check_multiplicity(amount):
        ...
        if count == COUNTER4PERCENTAGES:
            ...
            bank_account += bonus
            operations.append(f'Процент за пополнение {bonus} у.е. Итого {bank_account} у.е.')


def withdraw(amount):
    global bank_account, operations
    if check_multiplicity(amount):
        commission = min(max(amount * PERCENT_REMOVAL, MIN_REMOVAL), MAX_REMOVAL)
        total_withdraw = amount + commission
        if total_withdraw <= bank_account:
            bank_account -= total_withdraw
            operations.append(
                f'Снятие с карты {amount} у.е. Процент за снятие {commission:.0f} у.е.. Итого {bank_account:.0f} у.е.')
        else:
            operations.append(
                f'Недостаточно средств. Сумма с комиссией {total_withdraw:.0f} у.е. На карте {bank_account:.0f} у.е.')
            return
...
def exit():
    ...
    if bank_account > RICHNESS_SUM:
        ...
        bank_account -= tax
        operations.append(
            f'Вычтен налог на богатство {RICHNESS_PERCENT * 100}% в сумме {tax}. Итого {bank_account} у.е.')
    else:
